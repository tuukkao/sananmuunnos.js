...
var double_vowel = /^[^aeiouyäö]?([aeiouyäö])\1/,
    ...
    // Initial consonant: The word begins with one or more consonants and continues with
...
function is_initial_consonant(word1, word2) {
    ...
    if (initial_consonant.test(word1)) {
        var vowel1 = vowel.exec(word1),
            vowel2 = vowel.exec(word2);
        transformed1 = word2.slice(0, vowel2.index + 1) + word1.slice(vowel1.index + 1);
        transformed2 = word1.slice(0, vowel1.index + 1) + word2.slice(vowel2.index + 1);
        return([transformed1, transformed2]);
    } else {
        return(false);
    }
